...
// The per-recipient fan-out loop can outlive the default function
// timeout on Vercel — same allowance as the v1 broadcasts route.
...
export async function POST(request: Request) {
  try {
    ...
    // Resolve Twilio credentials once, up front — an incomplete Twilio
    // config should fail the request, not every recipient in the loop.
    ...
    let resolvedLanguage = template_language || 'en_US'
    try {
      const resolution = await resolveTemplateRowForSend({
        ...
        provider: config.provider === 'twilio' ? 'twilio' : 'meta',
      })
    ...
    // A Twilio send is keyed on the Content SID — reject before the
    // loop instead of failing every recipient identically.
    if (config.provider === 'twilio' && !templateRow?.twilio_content_sid) {
      return NextResponse.json(
        {
          error: `Template "${template_name}" has no Twilio Content SID — run template Sync in Settings before broadcasting.`,
        },
        { status: 400 },
      )
    }
